...
export interface IBookmarkAgent {
  ...
  updateBookmark(id: string, data: Partial<Omit<NewBookmark, "id">>): Promise<Bookmark>;
...
export function createBookmarkAgent(db: DB): IBookmarkAgent {
  ...
  return {
    addBookmark: async (data) => {
      const [row] = await q.insert(bookmarks).values({
        ...data,
        id: randomId("bkm"),
        dateAdded: now(),
        lastUpdatedAt: now(),
    ...
    getBookmark: async (id) => {
      const [row] = await q.select().from(bookmarks).where(eq(bookmarks.id, id)).limit(1);
      return row;
    },
    listBookmarks: async (collectionId) => {
      if (collectionId && collectionId !== "all") {
        return q.select().from(bookmarks).where(eq(bookmarks.collectionId, collectionId));
      }
      return q.select().from(bookmarks);
    },
    updateBookmark: async (id, data) => {
      const [row] = await q.update(bookmarks)
    ...
    toggleFavorite: async (id) => {
      const [cur] = await q.select({ favorite: bookmarks.favorite }).from(bookmarks).where(eq(bookmarks.id, id)).limit(1);
      if (cur) await q.update(bookmarks).set({ favorite: !cur.favorite, lastUpdatedAt: now() }).where(eq(bookmarks.id, id));
    },
    toggleLiked: async (id) => {
      const [cur] = await q.select({ liked: bookmarks.liked }).from(bookmarks).where(eq(bookmarks.id, id)).limit(1);
      if (cur) await q.update(bookmarks).set({ liked: !cur.liked, lastUpdatedAt: now() }).where(eq(bookmarks.id, id));
    },
    toggleSaved: async (id) => {
      const [cur] = await q.select({ saved: bookmarks.saved }).from(bookmarks).where(eq(bookmarks.id, id)).limit(1);
      if (cur) await q.update(bookmarks).set({ saved: !cur.saved, lastUpdatedAt: now() }).where(eq(bookmarks.id, id));
    },
    addTag: async (id, tag) => {
      const [cur] = await q.select({ tags: bookmarks.tags }).from(bookmarks).where(eq(bookmarks.id, id)).limit(1);
      if (!cur) throw new Error(`Bookmark ${id} not found.`);
      const existing: string[] = Array.isArray(cur.tags) ? cur.tags : [];
      if (existing.includes(tag)) return;
      await q.update(bookmarks).set({ tags: [...existing, tag], lastUpdatedAt: now() }).where(eq(bookmarks.id, id));
    },
  };
}
...
export interface IRssAgent {
  ...
  updateFeedMeta(id: string, meta: { title?: string; lastFetched?: string; unreadCount?: number }): Promise<void>;
...
export function createRssAgent(db: DB): IRssAgent {
  ...
  return {
    addFeed: async (data) => {
      const [row] = await q.insert(feeds).values({
        ...data,
        id: randomId("feed"),
        unreadCount: 0,
        lastUpdatedAt: now(),
    ...
    insertArticles: async (parsed) => {
      ...
      const existing = await q
        .select({ link: articles.link, id: articles.id, imageUrl: articles.imageUrl })
        .from(articles)
    ...
    updateFeedMeta: async (id, meta) => {
      await q.update(feeds)
        .set({ ...meta, lastUpdatedAt: now() })
        .where(eq(feeds.id, id));
    },
    listArticles: async (feedId) => {
      if (feedId) return q.select().from(articles).where(eq(articles.feedId, feedId));
    ...
    toggleArticleLike: async (id) => {
      const [cur] = await q.select({ liked: articles.liked }).from(articles).where(eq(articles.id, id)).limit(1);
      if (cur) await q.update(articles).set({ liked: !cur.liked, lastUpdatedAt: now() }).where(eq(articles.id, id));
    },
    toggleArticleSave: async (id) => {
      const [cur] = await q.select({ saved: articles.saved }).from(articles).where(eq(articles.id, id)).limit(1);
      if (cur) await q.update(articles).set({ saved: !cur.saved, lastUpdatedAt: now() }).where(eq(articles.id, id));
    },
