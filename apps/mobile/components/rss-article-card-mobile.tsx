...
function RssArticleCardMobileBase({
	...
	const imageSource = article.imageData || article.imageUrl;
	return (
		<Link
			href={{
				pathname: "/rss/[id]",
				...
				<Card className="gap-0 overflow-hidden py-0">
					<View className="flex-row">
						...
						{imageSource ? (
							<Image
								source={
									article.imageData
										? { uri: `data:image/jpeg;base64,${article.imageData}` }
										: { uri: imageSource }
								}
					...
					<View className="flex-row items-center justify-between border-border/50 border-t px-4 py-1">
						<View className="flex-row items-center gap-2">
							...
							<Text
								className="max-w-[42%] font-medium text-muted-foreground text-xs"
								numberOfLines={1}
							>
								{feedTitle}
							</Text>
